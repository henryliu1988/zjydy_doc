...
public class GridMonthView extends MonthView
{
    ...
    protected void drawText(Canvas canvas, int column, int row, int year, int month, int day)
    {
        ...
        float startY = rowSize * row + rowSize / 3 - (paint.ascent() + paint.descent()) / 2;
        paint.setStyle(Paint.Style.STROKE);
        ...
        {//日期为选中的日期
            ...
            {//desc不为空的时候
                ...
                int priceY = (int) (startY + theme.sizeDay() * 1.5);
                canvas.drawText(des, priceX, priceY, paint);

                paint.setTextSize(theme.sizeDay());
                int commentsX = (int) (columnSize * column + (columnSize - paint.measureText("项检查")) / 2);
                canvas.drawText("项检查", commentsX, (int) (priceY + theme.sizeDay() * 1.1), paint);
        ...
        } else
        {
            ...
            {//没选中，但是desc不为空
                ...
                paint.setTextSize(theme.sizeDay());
                int commentsX = (int) (columnSize * column + (columnSize - paint.measureText("项检查")) / 2);
                canvas.drawText("项检查", commentsX, (int) (priceY + theme.sizeDesc() * 1.1), paint);
            } else
            {//des为空
                paint.setColor(theme.colorWeekday());
                canvas.drawText(day + "", startX, startY, paint);
